...
11. If a PR-review run is already `awaiting_bot_review` but its worker launcher is absent/stale, reattach polling from controller A with `ai_dev_loop pr-review resume <run-id> --controller-session-id <exact-A>` — same run; may read PR/head for validation, but no GitHub writes, no second `@codex review`, and no Cursor/Codex invocation during the command.
12. If a historical PR-review successor is stuck in `waiting_for_user_attention` with `eligible_thread_set_drift` because an `external_adjudication` freeze from a prior cycle survived into a newer published cycle — either on the current run's direct recovery or via one verified terminal ancestor when the current recovery is `reviewing` — post a fresh exact `@rojobad /ai-dev-loop continue` on the PR and run `ai_dev_loop pr-review continue <run-id>`. That lineage-bound path clears only the obsolete freeze on the same run (same PR/SHA/marker/A/B/Cursor chat) and does not republish `@codex review`. A previously consumed continue comment requires a new one. Do not edit `state.json` by hand.
