...
9. For optional GitHub PR review after a completed A/B run, enable `pr_review_v2.enabled: true`, then use `ai_dev_loop pr-review create` or `prepare` (read-only) followed by `ai_dev_loop pr-review start` as the sole external-effects gate. Inspect progress with `pr-review status` and `pr-review history`.
10. Legacy v1 PR-review runs (`RunState.github_pr_review`) and retired subcommands (`continue`, `recover`, `set-cursor-model`, `pr-review-v2`) are not supported after Phase 16.9; there is no migration adapter.
