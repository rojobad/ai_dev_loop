...
11. If a historical PR-review successor is stuck in `waiting_for_user_attention` with `eligible_thread_set_drift` because an `external_adjudication` freeze from a prior cycle survived into a newer published cycle, post a fresh exact `@rojobad /ai-dev-loop continue` on the PR and run `ai_dev_loop pr-review continue <run-id>`. That lineage-bound path clears only the obsolete freeze on the same run (same PR/SHA/marker/A/B/Cursor chat) and does not republish `@codex review`. Do not edit `state.json` by hand.
