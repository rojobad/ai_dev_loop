...
10. If a PR-review run failed in `fixing_external_feedback` after an actionable external adjudication but before the fresh Cursor iteration started (empty staging over historical iteration artifacts), use `ai_dev_loop pr-review recover --dry-run <failed-run-id>` then `pr-review recover` / `pr-review resume` on the successor. That `external_feedback_cursor` path reuses the persisted adjudication and opens Cursor with the exact `github-NN` prompt before staging; it does not re-adjudicate or post another `@codex review`.
11. If a PR-review run is `interrupted` after Codex B already wrote `github/cycles/NN/*` (and optionally `prompts/fixes/github-NN.txt`) but before state pointers / remote preflight completed, use `ai_dev_loop pr-review resume <run-id> --controller-session-id <exact-A>` on the **same** run. Resume hydrates the durable `github_pr_review.external_adjudication` checkpoint for the current cycle only; it does not create a successor, re-adjudicate, or treat historical `recovery.source_prompt_sha256` as a global guard for later cycles.
12. If a PR-review run is already `awaiting_bot_review` but its worker launcher is absent/stale, reattach polling from controller A with `ai_dev_loop pr-review resume <run-id> --controller-session-id <exact-A>` — same run; may read PR/head for validation, but no GitHub writes, no second `@codex review`, and no Cursor/Codex invocation during the command.
13. If a historical PR-review successor is stuck in `waiting_for_user_attention` with `eligible_thread_set_drift` because an `external_adjudication` freeze from a prior cycle survived into a newer published cycle — either on the current run's direct recovery or via one verified terminal ancestor when the current recovery is `reviewing` — post a fresh exact `@rojobad /ai-dev-loop continue` on the PR and run `ai_dev_loop pr-review continue <run-id>`. That lineage-bound path clears only the obsolete freeze on the same run (same PR/SHA/marker/A/B/Cursor chat) and does not republish `@codex review`. A previously consumed continue comment requires a new one. Do not edit `state.json` by hand.
