...
def test_codex_review_result_schema_has_no_unique_items() -> None:
    path = schema_path("codex-review-result-v1.json")
    payload = json.loads(path.read_text(encoding="utf-8"))
    assert find_incompatible_response_format_keywords(payload) == []
    validate_codex_response_schema(path, schema_name="codex-review-result-v1.json")
