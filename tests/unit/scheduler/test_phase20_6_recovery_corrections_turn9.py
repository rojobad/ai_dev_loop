...
def _delta_integration_setup(
    ...
    artifacts.write_bytes(recovery_run_id, patch_rel, accepted_patch, max_bytes=len(accepted_patch) + 1)
    review_sha = "4" * 64
...
def test_delta_restart_finishes_worktree_when_index_already_accepted(
    ...
    store, artifacts, service, recovery_id, definition, accepted_tree, head, accepted_patch, source_patch = (
...
def test_non_final_sequence_abort_after_cas_preserves_hold_until_reconciled(
    ...
    with store.begin_immediate() as conn:
        store.release_checkpoint_reconciliation_hold(
            conn,
            run_id=source_run_id,
            intent_sha256=pending.integration_intent_artifact_sha256,
        )
