...
def _ensure_pre_cas_hold(
    ...
    *,
    ...
    with store.begin_read() as conn:
        assert store.has_checkpoint_reconciliation_hold(conn, definition.source_run_id)
    ...
    for _ in range(30):
        ...
        with store.begin_read() as conn:
            ...
            if state_kind == ABORT_PENDING_ROLLOVER_STATE_KIND:
                saw_abort_pending = True
            if state_kind == ABORTED_ROLLOVER_STATE_KIND:
                break
    else:
        raise AssertionError(
            "rollover did not converge to aborted after pre-CAS abort reconciliation"
        )
    assert saw_abort_pending
    assert checkpoint_git_rev_parse(repo, "refs/heads/main") == head


def test_pre_cas_abort_retains_hold_on_foreign_intent_digest(
    tmp_path: Path,
    isolated_xdg: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    with store.begin_immediate() as conn:
        conn.execute(
            """
            UPDATE scheduler_checkpoint_holds
            SET intent_sha256 = ?
            WHERE run_id = ?
            """,
            ("f" * 64, source_run_id),
        )
    RolloverAbortService(store, artifacts).abort(rollover_id)
    tick = TickService(
        store,
        artifacts,
        FakeGitAdmissionPort(resolved_root=str(definition.repository.root)),
        now_factory=service._now_factory,
        tick_owner_factory=lambda: "tick-owner-foreign-hold",
        attempt_backend=FakeAgentProcessBackend(
            default_scenario=FakeAttemptScenario(active_ticks=0, exit_code=0)
        ),
    )
    ...
    with store.begin_read() as conn:
        ...
        row = store.get_authenticated_rollover(conn, rollover_id)
    assert row is not None
    assert str(row["state_kind"]) == INTEGRATION_PENDING_ROLLOVER_STATE_KIND
    assert checkpoint_git_rev_parse(Path(definition.repository.root), "refs/heads/main") == head


def test_pre_cas_abort_retains_hold_on_missing_intent_artifact(
    tmp_path: Path,
    isolated_xdg: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    tick = TickService(
        ...
        FakeGitAdmissionPort(resolved_root=str(definition.repository.root)),
        now_factory=service._now_factory,
        tick_owner_factory=lambda: "tick-owner-missing-intent",
        attempt_backend=FakeAgentProcessBackend(
            default_scenario=FakeAttemptScenario(active_ticks=0, exit_code=0)
        ),
    )
...
def test_pre_cas_abort_retains_hold_on_corrupt_intent_artifact(
    tmp_path: Path,
    isolated_xdg: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    tick = TickService(
        ...
        FakeGitAdmissionPort(resolved_root=str(definition.repository.root)),
        now_factory=service._now_factory,
        tick_owner_factory=lambda: "tick-owner-corrupt-intent",
        attempt_backend=FakeAgentProcessBackend(
            default_scenario=FakeAttemptScenario(active_ticks=0, exit_code=0)
        ),
    )
