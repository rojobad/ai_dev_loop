...
4. Defaults seguros del paquete.

Durante `prepare`, `ai_dev_loop` persiste tanto la configuracion fuente como la configuracion efectiva.

## Modelo de review

El modelo de review debe existir para la cuenta de Codex que ejecutara `codex exec resume`.

En la workstation validada, `o4-mini` fallo para una cuenta enlazada a ChatGPT y `gpt-5.5` funciono. No asumas que el default de ejemplo sirve para todas las cuentas. Verifica con:
