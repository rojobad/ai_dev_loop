...
AUTHENTICATED_ROLLOVER_STARTED_EVENT_KIND = "authenticated_rollover_started"
...
class AuthenticatedRolloverStartedEvent(DomainModel):
    kind: str = Field(default=AUTHENTICATED_ROLLOVER_STARTED_EVENT_KIND)
    ...
    @field_validator("kind")
    @classmethod
    def kind_is_authenticated_rollover_started(cls, value: str) -> str:
        if value != AUTHENTICATED_ROLLOVER_STARTED_EVENT_KIND:
            raise ValueError("kind must be authenticated_rollover_started")
        return value
...
SchedulerEvent = Annotated[
    ...
    | Annotated[
        ...
        Tag(AUTHENTICATED_ROLLOVER_STARTED_EVENT_KIND),
    ]
