...
DEFAULT_ATTEMPT_RUNTIME_SECONDS = 3600
_SHOW_PROPERTIES = (
    "Id",
    "LoadState",
    "ActiveState",
...
class SystemdUserBackend(AgentProcessBackend):
    ...
    attempt_runtime_seconds: int = DEFAULT_ATTEMPT_RUNTIME_SECONDS
    ...
    def launch(self, request: LaunchRequest) -> None:
        ...
        args = [
            *self.systemd_run,
            ...
            f"--property=RuntimeMaxSec={int(self.attempt_runtime_seconds)}",
            f"--property=TimeoutStopSec={int(self.attempt_runtime_seconds)}",
            "--",
            *wrapped,
        ]
