...
class TickService:
    def __init__(
        ...
        *,
        ...
        self._rollover_reconcile: RolloverReconcileService | None = None
        ...
        if attempt_backend is not None:
            ...
            self._rollover_reconcile = RolloverReconcileService(
                store,
                artifacts,
                abort_backend=attempt_backend,
                now_factory=self._now_factory,
        ...
        try:
            ...
            if lease_acquired:
                with self.store.begin_read() as conn:
                    ...
                    if self._rollover_reconcile is not None:
                        receipts.extend(self._rollover_reconcile.reconcile_pending_rollovers(conn))
            ...
            if self._rollover_reconcile is not None:
                receipts.extend(self._rollover_reconcile.finalize_deferred_aborts())
                receipts.extend(self._rollover_reconcile.finalize_deferred_integrations())
                receipts.extend(self._rollover_reconcile.finalize_deferred_cleanups())
