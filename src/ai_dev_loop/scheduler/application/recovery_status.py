"""Read-only fresh-review recovery status for Phase 20.6."""
...
from ai_dev_loop.scheduler.application.contracts import (
    ...
    integrated_recovery_safe_next_action,
...
def _safe_action_for_kind(recovery_id: str, state_kind: str) -> SafeNextAction:
    if state_kind == PREPARED_RECOVERY_STATE_KIND:
        return prepared_recovery_start_next_action(recovery_id)
    if state_kind in {
        ACTIVE_RECOVERY_STATE_KIND,
        INTEGRATION_PENDING_RECOVERY_STATE_KIND,
        CLEANUP_PENDING_RECOVERY_STATE_KIND,
    }:
        return active_recovery_tick_next_action(recovery_id)
    if state_kind == INTEGRATED_RECOVERY_STATE_KIND:
        return integrated_recovery_safe_next_action(recovery_id)
    return SafeNextAction(
        kind=SafeNextActionKind.INSPECT_BLOCKED,
        command=f"Recovery {recovery_id} is in state {state_kind}. Inspect protected artifacts.",
    )


def recovery_status(recovery_id: str, *, db_path: Path | None = None) -> RecoveryStatusResult:
    ...
    try:
        ...
        raise SchedulerEngineError(
            ...
            "recovery status requires a readable scheduler schema",
        ) from exc
    ...
    return RecoveryStatusResult(
        ...
        recovery_id_prefix=recovery_id[:12],
        source_run_id_prefix=str(row["source_run_id"])[:8],
        state_kind=state_kind,
        recovery_run_id_prefix=(
            str(row["recovery_run_id"])[:8] if row["recovery_run_id"] else None
        ),
        ...
        safe_next_action=_safe_action_for_kind(recovery_id, state_kind),
    )
