"""Managed rollover worktree port for Phase 20.6.5."""
...
class RolloverWorktreeError(ValidationError):
    """Raised when a rollover worktree operation cannot complete safely."""


@dataclass(frozen=True)
class RolloverSeedEvidence:
    parent_head: str
...
class RolloverWorktreePort(Protocol):
    def create_and_seed(
        self,
    ...
    def remove_worktree(
        ...
        *,
        ...
        """Remove an exact owned rollover worktree."""
    ...
    def delete_private_ref(
        ...
        *,
        ...
        """Delete the private rollover ref after identity verification."""


class ProductionRolloverWorktreePort:
    def create_and_seed(
        self,
        definition: AuthenticatedRolloverDefinition,
        *,
        ...
        if pre_seed.status_porcelain.strip():
            raise RolloverWorktreeError("rollover worktree is not clean before patch seeding")
        patch_bytes = patch_path.read_bytes()
        if hashlib.sha256(patch_bytes).hexdigest() != definition.source_staged_patch_sha256:
            raise RolloverWorktreeError("patch artifact digest mismatch")
    ...
    def _reconcile_partial_worktree(
        ...
        *,
        ...
        if private_sha is None:
            ...
            raise RolloverWorktreeError("partial rollover private ref drift")
        if not recovery_git_worktree_contains(repo_root, worktree_path=worktree_path):
            raise RolloverWorktreeError("partial rollover worktree is not registered")
        pre_seed = discover_repository_bounded(worktree_path)
        checkpoint_validate_repository_layout(
            worktree_path,
            ...
            context="partial rollover worktree identity",
        )
        head = checkpoint_git_rev_parse(worktree_path, "HEAD")
        if head != definition.parent_head:
            raise RolloverWorktreeError("partial rollover worktree HEAD drift")
        live_patch = checkpoint_git_diff_cached_patch_bytes(worktree_path)
        live_patch_sha = hashlib.sha256(live_patch).hexdigest()
        tree_sha = checkpoint_git_write_tree(worktree_path)
        patch_bytes = patch_path.read_bytes()
        ...
        if live_patch_sha != definition.source_staged_patch_sha256:
            if pre_seed.status_porcelain.strip():
                raise RolloverWorktreeError("partial rollover worktree is not clean before reseed")
            recovery_git_apply_staged_patch(worktree_path, patch_bytes=patch_bytes)
            live_patch = checkpoint_git_diff_cached_patch_bytes(worktree_path)
            live_patch_sha = hashlib.sha256(live_patch).hexdigest()
            ...
            recovery_git_apply_staged_patch(worktree_path, patch_bytes=patch_bytes)
        if live_patch_sha != definition.source_staged_patch_sha256:
            raise RolloverWorktreeError("partial rollover seeded patch hash mismatch")
        checkpoint_validate_staged_patch_matches_artifact(worktree_path, patch_path)
        tree_sha = checkpoint_git_write_tree(worktree_path)
        if tree_sha != definition.source_staged_tree_sha256:
            raise RolloverWorktreeError("partial rollover seeded tree SHA mismatch")
        status = checkpoint_git_status_porcelain(worktree_path)
        if paths_with_unstaged_changes(status) or paths_with_untracked(status):
            raise RolloverWorktreeError("partial rollover worktree is not clean after reconcile")
        post_seed = discover_repository_bounded(worktree_path)
        return RolloverSeedEvidence(
            parent_head=definition.parent_head,
            staged_patch_sha256=live_patch_sha,
...
class FakeRolloverWorktreePort:
    """In-memory fake for hermetic tests."""

    def __init__(
        self,
        *,
        on_create: Callable[..., RolloverSeedEvidence] | None = None,
    ...
    def create_and_seed(
        ...
        *,
        ...
        if self._fail_on == "create":
            raise RolloverWorktreeError("injected worktree failure")
        self.created.append((definition.rollover_id, worktree_path))
