...
def _resolve_sequence_binding(
    ...
    *,
    ...
    if isinstance(sequence_state, ActiveSequenceState):
    ...
    elif isinstance(sequence_state, BlockedSequenceState):
        if sequence_state.current_run_id != source_run_id:
            raise SchedulerEngineError(
                SchedulerEngineErrorKind.VALIDATION,
        ...
        if sequence_state.block_reason_kind != ROLLOVER_SEQUENCE_BLOCK_REASON:
            raise SchedulerEngineError(
                SchedulerEngineErrorKind.VALIDATION,
                "blocked sequence reason is not rollover-eligible",
            )
    else:
        raise SchedulerEngineError(
            SchedulerEngineErrorKind.VALIDATION,
            "sequence source must be the current active or rollover-blocked sequence phase",
        )
