...
**already-open PR** (not created from a completed `ai_dev_loop` source run), B
may instead run the read-only v2 prepare command:
...
Return the prepared run id to A. A alone opens the external-effects gate with
`ai_dev_loop pr-review start <run-id>`; do not use retired v1 flags,
`set-cursor-model`, `continue`, or `recover`.
